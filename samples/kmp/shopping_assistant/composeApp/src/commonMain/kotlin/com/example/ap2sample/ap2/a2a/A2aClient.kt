...
class A2aClient(val name: String, val baseUrl: String, val agentCard: AgentCard? = null) {
            ...
            logger =
                    object : Logger {
                        override fun log(message: String) {
                            PlatformLogger.d(TAG, "Ktor Log: $message")
                        }
                    }
            level = LogLevel.ALL
        }
    ...
    suspend fun sendMessage(message: Message): JsonObject {
        PlatformLogger.d(TAG, "[$name] Preparing to send message: ${message.messageId}")
        try {
            ...
            PlatformLogger.d(TAG, "[$name] Response received successfully: $response")
            return response
        } catch (e: Exception) {
            PlatformLogger.e(TAG, "[$name] Failed to send data", e)
            throw e
        }
    }
    ...
    companion object {
        ...
        suspend fun setUpClient(name: String, url: String): A2aClient {
            try {
                ...
                PlatformLogger.d(TAG, "Fetching agent card from: $agentCardUrl")
                val card: AgentCard = httpClient.get(agentCardUrl).body()
                return A2aClient(name, url, card)
            } catch (e: Exception) {
                PlatformLogger.e(TAG, "Failed to fetch agent card", e)
                throw e
            }
        }
    }
